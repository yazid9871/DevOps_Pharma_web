...
Force Tags        Flow de création offre groupée    Freemium
...
${INFOBULLE_LANCER_MARCHE_FERMER}  xpath=//button[.//span[contains(text(),"D'accord, compris")]]
...
Accéder à la page de création d'offre groupée
    ...
    ...                avec un fournisseur pré-rempli via le paramètre supplier_id, depuis le
    ...                compte freemium.
...
Vérifier que le fournisseur est pré-rempli
    ...
    ...                l'URL contient supplier_id=65 (vérifié en direct sur le compte freemium -
    ...                comportement identique au compte ADMIN).
    Aller à la page de création d'offre groupée
...
Vérifier que le titre de l'offre est requis
    ...
    ...                "Ce champ est requis" (vérifié en direct sur le compte freemium).
    Aller à la page de création d'offre groupée
...
Vérifier la saisie du titre de l'offre
    ...
    Aller à la page de création d'offre groupée
    Input Text    ${FIELD_NOM}    offre de test automation freemium
    ${valeur}    Get Value    ${FIELD_NOM}
    Should Be Equal As Strings    ${valeur}    offre de test automation freemium
...
Vérifier l'ouverture du pop-up de remise globale
    ...
    ...                remise (vérifié en direct sur le compte freemium).
...
Vérifier le résumé affiché après enregistrement de la remise globale
    ...
    ...                s'affiche après enregistrement (vérifié en direct : "5 U, 1000 DHS  = 10 %").
    Ouvrir le pop-up de remise globale
    Sélectionner le type de remise globale    Remise financière
    Ajouter une ligne de palier global    ${QTE_INPUT_1}    ${MONTANT_INPUT_1}    ${REMISE_INPUT_1}    5    1000    10
    Enregistrer la remise globale
...
Vérifier la saisie des termes et conditions
    ...
    Aller à la page de création d'offre groupée
    Input Text    ${FIELD_TERMES_CONDITIONS}    Conditions de test automation freemium
    ${valeur}    Get Value    ${FIELD_TERMES_CONDITIONS}
    Should Be Equal As Strings    ${valeur}    Conditions de test automation freemium
...
Vérifier l'ouverture de la sélection de produits pour un pack
    ...
    ...                tableau de sélection de produits (10 lignes constatées en direct sur le
    ...                compte freemium).
...
Vérifier la saisie du champ Obligatoire pour passer commande
    [Documentation]    Vérifie que le champ "requiredToPlaceOrder" accepte une saisie (vérifié
    ...                en direct sur le script de référence : valeur "obligatoire").
    Ouvrir la sélection de produits pour un pack
    Sélectionner des lignes de produits    1    4
    click element    ${SELECTIONNER_BUTTON}
    click element    ${PARAMETRES_PACK_BUTTON}
...
Vérifier que le champ Ciblage de l'offre accepte une recherche
    ...
    ...                option peut être sélectionnée via ENTER.
    Aller à la page de création d'offre groupée
...
*** Keywords ***
Aller à la page de création d'offre groupée
    [Documentation]    Navigue vers la page de création d'offre groupée avec un fournisseur
    ...                pré-rempli (via le paramètre supplier_id) après connexion, puis ferme
    ...                l'infobulle "Lancer un marché" si elle s'affiche (constatée en direct au
    ...                premier chargement de la page sur le compte freemium, disparaît ensuite
    ...                définitivement).
    Go To    ${BASE_URL}/${CREATE_OFFER_URL}
    Wait Until Element Is Visible    ${FIELD_NOM}    timeout=30s
    Run Keyword And Ignore Error    Click Element    ${INFOBULLE_LANCER_MARCHE_FERMER}
...
Remplir le formulaire d'offre groupée
    ...
    Aller à la page de création d'offre groupée
    Input Text    ${FIELD_NOM}    offre de test automation freemium complète
    Wait Until Element Is Visible    ${FIELD_CIBLAGE}    10s
    Click Element    ${FIELD_CIBLAGE}
    Input Text    ${FIELD_CIBLAGE}    groupe
    Sleep    2s
