...
Force Tags        Sobrus Marketplace    Freemium
...
${TAB_TOUTES_LES_OFFRES}           xpath=//div[contains(concat(' ', normalize-space(@class), ' '), ' sob-v2-tab ')][.//span[text()="Toutes les offres"]]
...
Cliquer sur le tab Toutes les offres et vérifier qu'il devient actif
    ...
    ...                en direct : le tab actif porte la classe "sob-v2-tab active" - le
    ...                locator doit cibler précisément le token de classe "sob-v2-tab" et non
    ...                le conteneur parent "sob-v2-tabs-container", qui contient aussi cette
    ...                sous-chaîne).
...
Vérifier l'affichage de l'avatar labo Laboratoire ORGACOS
    ...
    ...                direct : 5 laboratoires réels affichés, nom porté par l'attribut
    ...                "title" du bouton et non par un "aria-label" ni le texte visible).
...
Cliquer sur l'avatar labo Laboratoire ORGACOS et vérifier la navigation
    ...
    ...                fournisseur correspondant (hors de la page /offers, vérifié en direct :
    ...                URL offers/supplier/{id}/offers).
...
Vérifier les valeurs par défaut des filtres
    ...
    ...                (vérifié en direct sur le compte freemium : min 1 / max 50 / quantité
    ...                1234 / montant 100000 / remise 80).
    Ouvrir le panneau de filtres
...
Vérifier le filtre Nombre de produits min avec la valeur 50
    [Documentation]    Vérifie que le champ "Nombre de produits dans l'offre" (min) accepte la
    ...                valeur 50 (borne max par défaut sur le compte freemium).
    Saisir une valeur de filtre et vérifier    ${NB_PRODUITS_MIN_INPUT}    50
...
Vérifier le filtre Nombre de produits max avec la valeur 50
    ...
    ...                valeur 50 (valeur par défaut sur le compte freemium).
...
Vérifier le filtre Nombre de produits max avec la valeur 60
    [Documentation]    Vérifie que le champ "Nombre de produits dans l'offre" (max) accepte une
    ...                valeur au-delà de la borne par défaut (60 > 50).
    Saisir une valeur de filtre et vérifier    ${NB_PRODUITS_MAX_INPUT}    60
...
Vérifier le filtre Remise maximal possible avec la valeur 80
    [Documentation]    Vérifie que le champ "Remise maximal possible" accepte sa valeur par
    ...                défaut sur le compte freemium (80).
    Saisir une valeur de filtre et vérifier    ${REMISE_MAX_INPUT}    80
...
Vérifier l'affichage du badge de réduction
    ...
    ...                carte d'offre. Sur le compte freemium, ce badge est absent des ~15
    ...                premières cartes chargées sans défilement (vérifié en direct) : un
    ...                défilement est donc nécessaire pour charger davantage de cartes avant
    ...                la vérification.
...
Vérifier le nombre total d'offres affiché sur le tab Toutes les offres
    ...
    ...                (vérifié en direct sur le compte freemium : "84").
...
Vérifier l'affichage du tab par son nom
    [Arguments]    ${NOM_TAB}
    Aller à la page Sobrus Marketplace
    Wait Until Element Is Visible    xpath=//div[contains(concat(' ', normalize-space(@class), ' '), ' sob-v2-tab ')][.//span[text()="${NOM_TAB}"]]    10s

Cliquer sur le tab par son nom
    [Arguments]    ${NOM_TAB}
    ${tab_locator}    Set Variable    xpath=//div[contains(concat(' ', normalize-space(@class), ' '), ' sob-v2-tab ')][.//span[text()="${NOM_TAB}"]]
...
Vérifier l'affichage de l'avatar labo par son nom
    [Arguments]    ${NOM_LABO}
    Aller à la page Sobrus Marketplace
    Wait Until Element Is Visible    xpath=//button[@title="${NOM_LABO}" or contains(@title, "${NOM_LABO}") or @aria-label="${NOM_LABO}" or contains(., "${NOM_LABO}")]    10s

Cliquer sur l'avatar labo et vérifier la navigation
    [Arguments]    ${NOM_LABO}
    Aller à la page Sobrus Marketplace
    ${avatar_locator}    Set Variable    xpath=//button[@title="${NOM_LABO}" or contains(@title, "${NOM_LABO}") or @aria-label="${NOM_LABO}" or contains(., "${NOM_LABO}")]
