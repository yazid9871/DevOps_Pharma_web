...
Vérifier La Visibilité Et La Fonctionnalité De L'Icône Éditer
    ...
    Vérifier Fonctionnalité Icône Éditer     Modifier le contac      suppliers/contacts
Vérifier Le Boutons "créer Contacts"
    [Documentation]    Vérifie la présence des boutons d'actions globales sur la page Contacts
   Vérifier Bouton créer / Suggérer     ${BOUTON_CREER}    Créer un nouveau contact     suppliers/contacts
...
Rechercher Par nom Fournisseurs
    [Documentation]    Vérifie la recherche d'un Contact par Nom de Fournisseur
    Cliquer Sur Actualiser
    Vérifier Que Le Champ De Recherche Est Vide
   Vérifier La Recherche     td[5]    kjgljg    ${CHAMP_FOURNISSEUR_CONTACT}
