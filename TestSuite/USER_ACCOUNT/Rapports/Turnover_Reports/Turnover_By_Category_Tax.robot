...
*** Settings ***
Documentation     Tests fonctionnels de la page "Rapports Chiffre d'affaires par famille tarifaire (Catégorie et TVA)"
...
${DATE_END}         28/02/2026
${TITRE_PAGE}            Chiffre d'affaires par famille tarifaire (Catégorie et TVA)
${refresh}      css=.sob-v2-btn-tertiary
*** Test Cases ***

UC01 - Consulter le rapport
     [Documentation]    Vérifier affichage du rapport
      Accéder à la page     reports/turnoverbycategoryandtax
...
UC03 - Regrouper les données par jour
    [Documentation]    Vérifier checkbox regroupement
...
UC07 - Cas sans données
    [Documentation]    Vérifier comportement sans données
    Input Text    xpath=//input[contains(@placeholder,'Date de début')]    01/01/1900
    Input Text    xpath=//input[contains(@placeholder,'Date de fin')]      02/01/1900
    Click Button   ${refresh}
