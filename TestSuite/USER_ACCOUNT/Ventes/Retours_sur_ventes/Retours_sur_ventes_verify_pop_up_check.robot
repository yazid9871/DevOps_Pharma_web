*** Settings ***
Documentation     Tests fonctionnels de la page "Page de création de Retours_sur_ventes :verify_pop_up_check"
...
Force Tags        Page de création de Retours_sur_ventes : verify_pop_up_check
...
go to create devis page
    Go To devis Listing Page    salesreturn/create?source=sales_return_saved
    go to create devis page
...
ouvrire pop up contact
    Cliquer sur le champ     id=contact_name
...
Vérifier que le contact sélectionné est correct
    Vérifier que le X sélectionné est correct        id=contact_name
