*** Settings ***
Documentation     Tests fonctionnels de la page "Page de création de ventes"
...
Force Tags        Page de création de ventes
...
${SWITCH_NOUVELLE_INTERFACE}    css=.sob-v2-switch-slider-container
...
Vérifier le choix du contact
     [Documentation]    Vérifie qu'après la sélection d'un client, le champ "Contact" apparaît,
     ...                ouvre le popup de sélection des contacts et que le contact sélectionné
     ...                est correctement affiché.
     Cliquer sur le champ Client
     Sélectionner le client dans le popup
     Cliquer sur le champ Contact
     Sélectionner le contact dans le popup
     Vérifier que le contact sélectionné est correct
...
Vérifier que le client sélectionné est correct
    ...
    ${nom}=    Get Value    id=customer_name
    Should Not Be Empty    ${nom}

Cliquer sur le champ Contact
    Wait Until Element Is Visible    id=contact_name    10s
    click element    id=contact_name
    Wait Until Element Is Visible    css=tr.zoom:nth-child(1)     10s
...
Vérifier que le contact sélectionné est correct
    ...
    ${nom}=    Get Value    id=contact_name
    Should Not Be Empty    ${nom}
Vérifie l'état du switch "Nouvelle interface de vente"
    [Documentation]    Vérifie l'état du switch "Nouvelle interface de vente" : si actif, clique
    ...                directement sur "créer" ; sinon, active le switch avant de cliquer sur "créer".
    ...                Attend ensuite l'affichage du bouton "brouillon".
    ${is_active}=    Run Keyword And Return Status    Checkbox Should Be Selected    ${CASHIER_MODE_CHECKBOX}
    IF    not ${is_active}
        Click Element    ${SWITCH_NOUVELLE_INTERFACE}
        sleep    1s
    END
