...
${SEARCH_FIELD_status}    id=is_delivered.q
${AD_SEARCH_FIELD_status}    id=is_delivered.operation
...
Search By not equal
    ...
    Verify Search Results delivry statu by not equal
...
Select statu and Verify Search Results
    FOR  ${i}  IN RANGE  1  3
         ...
         Execute JavaScript  document.querySelector("#react-select-2-option-${i}").scrollIntoView(true);
         sleep    2s
        click element    css:#react-select-2-option-${i}
         Click Element    ${SEARCH_FIELD_status}
            Wait Until Element Is Visible  ${status_DROPDOWN}    10s
           ${value} =  Get Text  css:#react-select-2-option-${i}
...
Verify Search Results delivry statu by not equal
    click element    css:#react-select-9-option-0
       Click Element    ${SEARCH_FIELD_status}
         Wait Until Element Is Visible  ${status_DROPDOWN}    10s
       FOR  ${i}  IN RANGE  1  3
