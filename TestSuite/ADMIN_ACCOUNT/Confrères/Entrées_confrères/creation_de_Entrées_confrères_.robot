*** Settings ***
Documentation     Tests fonctionnels de la page " Page de création de Entrées_confrères sans confrères"
...
Force Tags         Page de création de Entrées_confrères sans confrères
*** Variables ***
...
${amount}     0
*** Test Cases ***
Accéder À La Page De Création D'une Entrée Confrère
        Go To Entrées_confrères Listing Page       colleagues/purchases/create?pricing_field=sale_price
Vérifier Que La Date De L'Entrée Confrère Est Valide
      Verify Date Field Is Valid
Soumettre Une Entrée Confrère Vide
    submit Entrées_confrères
verify pop up empty Entrées_confrères
    verify pop up empty Entrées_confrères
...
Cliquer Sur Sauvegarder
    Cliquer Sur Sauvegarder
Vérifier Que L'Entrée Confrère A Été Ajoutée Avec Succès
    Vérifier Que L'Entrée Confrère A Été Ajoutée Avec Succès
...
Go To Entrées_confrères Listing Page
...
verify pop up empty Entrées_confrères
    wait until page contains    Vous devez choisir au moins un produit avec une quantité supérieur à 0.     5s
     ...
     sleep     3s
Verify Date Field Is Valid
     [Documentation]    Comme pour Avoirs fournisseurs émis (et contrairement à Vente/
     ...                id=invoice_date en DD/MM/YYYY), le champ Date d'une Entrée confrère a
     ...                l'id "date" et un format YYYY-MM-DD. Un DIV englobant partage aussi cet
     ...                id="date" (avant l'input dans le DOM) : on cible donc l'input réel avec
     ...                "css=#date input" pour éviter que le locator "id=date" ne résolve vers
     ...                le DIV.
       sleep    1s
...
Cliquer Sur Sauvegarder
    [Documentation]    Une fois le confrère renseigné, Sauvegarder enregistre l'entrée confrère
    ...                en statut Brouillon (comme Avoirs fournisseurs émis ; contrairement à
    ...                Retours sur ventes, où "Sauvegarder" mène directement à un écran de
    ...                paiement réel).
...
Vérifier Que L'Entrée Confrère A Été Ajoutée Avec Succès
    [Documentation]    Message réel confirmé en direct : "L'entrée du confrère a été sauvegardé
    ...                avec succès" (sans point d'exclamation, contrairement à Avoirs
    ...                fournisseurs émis). On vérifie un extrait sans l'apostrophe typographique
    ...                pour éviter tout souci d'encodage.
    sleep    1s
    wait until page contains    a été sauvegardé avec succès      10s

Aller À La Page Détails Et Vérifier Le Statut
    [Arguments]    ${statut_attendu}
    wait until element is visible    xpath=//*[@data-testid="afficher_l'entrée_confrère"]    10s
    click element    xpath=//*[@data-testid="afficher_l'entrée_confrère"]
    sleep    2s
