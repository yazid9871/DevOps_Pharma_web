...
Force Tags        Passer une offre groupée
...
${MODAL_GROUPE}                     css=.sob-v2-modal-container
${MODAL_GROUPE_CLOSE_BUTTON}        css=.sob-v2-modal-container button
...
# ---------------------------------------------------------------------------
# Groupe J - Offre groupée : commande de groupe non générée (6)
# ---------------------------------------------------------------------------
Vérifier l'ouverture du pop-up sur une offre groupée sans commande générée
    [Documentation]    Vérifie que le clic sur "Lancer la commande" d'une offre groupée sans
    ...                commande de groupe déjà générée affiche un pop-up d'information
    ...                (vérifié en direct sur l'offre "Cooper GPF").
    Aller à la page des offres
    Wait Until Element Is Visible    ${LANCER_COMMANDE_CARD_BUTTON}    10s
    click element    ${LANCER_COMMANDE_CARD_BUTTON}
    sleep    2s
    Wait Until Element Is Visible    ${MODAL_GROUPE}    10s

Vérifier le message Aucune commande groupée n'a encore été générée
    [Documentation]    Vérifie le texte exact du message d'information (vérifié en direct).
    Wait Until Page Contains    Aucune commande groupée n'a encore été générée pour cette offre    10s

Vérifier le message invitant à contacter les administrateurs du groupe
    [Documentation]    Vérifie le message invitant à contacter les administrateurs du groupe
    ...                (vérifié en direct).
    Wait Until Page Contains    Veuillez contacter les administrateurs de votre groupe    10s

Vérifier l'affichage de la liste des adhérents du groupe
    [Documentation]    Vérifie que la liste "Adhérés du groupe" est affichée dans le pop-up
    ...                (vérifié en direct : plusieurs pharmacies listées).
    Wait Until Page Contains    Adhérés du groupe    10s

Vérifier la fermeture du pop-up d'offre groupée
    [Documentation]    Vérifie que le pop-up peut être fermé via son bouton de fermeture.
    Wait Until Element Is Visible    ${MODAL_GROUPE_CLOSE_BUTTON}    10s
    click element    ${MODAL_GROUPE_CLOSE_BUTTON}
    sleep    1s
    Page Should Not Contain Element    ${MODAL_GROUPE}

Vérifier que la page reste sur la liste des offres après fermeture du pop-up
    [Documentation]    Vérifie qu'après fermeture du pop-up d'offre groupée, l'URL reste sur
    ...                la liste des offres (pas de navigation intempestive).
    click element    ${MODAL_GROUPE_CLOSE_BUTTON}
    sleep    1s
    Location Should Contain    offers
...
Aller à la page de commande d'une offre standard
    [Documentation]    Navigue vers la liste des offres puis clique sur "Commander" d'une
    ...                offre standard pour atteindre la page de sélection de quantités.
    Aller à la page des offres
    Wait Until Element Is Visible    ${COMMANDER_CARD_BUTTON}    10s
    click element    ${COMMANDER_CARD_BUTTON}
    sleep    2s
